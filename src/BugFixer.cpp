            ...
            for (Instruction &i : b) {
                ...
                switch (f->getIntrinsicID()) {
                    case Intrinsic::memcpy: {
                        ...
                        if (ExtraDumb) f = fixer->getPersistentVersion("memcpy_naive");
                        replace_map[cb] = f;
                        continue;
                    }
                    case Intrinsic::memset: {
                        Function *f = fixer->getPersistentVersion("memset");
                        if (ExtraDumb) f = fixer->getPersistentVersion("memset_naive");
                        replace_map[cb] = f;
                        continue;
                    }
                    case Intrinsic::memmove: {
                        Function *f = fixer->getPersistentVersion("memmove");
                        if (ExtraDumb) f = fixer->getPersistentVersion("memmove_naive");
                        replace_map[cb] = f;
                        continue;
                    }
                    default: {
                ...
                if (std::string::npos != fname.find("strncpy")) {
                    ...
                    if (ExtraDumb) f = fixer->getPersistentVersion("strncpy_naive");
                    replace_map[cb] = f;
                }
            }
        }
